    ...
    for (plist = pid_list; plist != NULL; plist = plist->next) {
        ...
        if (DISPLAY_IO(actflag)) {
            ...
            cprintf_u64(NO_UNIT, 1, 7,
                        (unsigned long long) (pstc->blkio_swapin_delays -
                                              pstp->blkio_swapin_delays));
        }
    ...
    for (plist = pid_list; plist != NULL; plist = plist->next) {
        ...
        if (DISPLAY_MEM(actflag)) {
            cprintf_u64(NO_UNIT, 2, 9,
                        (unsigned long long) ((pstc->minflt + pstc->cminflt) -
                                              (pstp->minflt + pstp->cminflt)),
                        (unsigned long long) ((pstc->majflt + pstc->cmajflt) -
                                              (pstp->majflt + pstp->cmajflt)));
        }
