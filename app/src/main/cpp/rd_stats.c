    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        if (line[0] == '/') {
            ...
            */
            ...
            st_filesystem_i->f_blocks =
                    (unsigned long long) buf.f_blocks * (unsigned long long) buf.f_frsize;
            st_filesystem_i->f_bfree =
                    (unsigned long long) buf.f_bfree * (unsigned long long) buf.f_frsize;
            st_filesystem_i->f_bavail =
...
int read_softnet(struct stats_softnet *st_softnet, __nr_t nr_alloc,
    ...
    while (fgets(line, sizeof(line), fp) != NULL) {
        ...
        while ((!(online_cpu_bitmap[(cpu - 1) >> 3] & (1 << ((cpu - 1) & 0x07)))) &&
               (cpu <= NR_CPUS + 1)) {
            cpu++;
        }
        if (cpu > NR_CPUS + 1)
