    ...
    if (DISPLAY_SHORT_OUTPUT(flags)) {
        ...
        if ((hpart == 1) || !hpart) {
            ...
            xios.rarqsz = (ioi->rd_ios - ioj->rd_ios) ?
                          (ioi->rd_sectors - ioj->rd_sectors) /
                          ((double) (ioi->rd_ios - ioj->rd_ios)) :
                          0.0;
        }
        if ((hpart == 2) || !hpart) {
            /* %wrqm */
            ...
            xios.warqsz = (ioi->wr_ios - ioj->wr_ios) ?
                          (ioi->wr_sectors - ioj->wr_sectors) /
                          ((double) (ioi->wr_ios - ioj->wr_ios)) :
                          0.0;
        }
        if ((hpart == 3) || !hpart) {
            /* %drqm */
            ...
            xios.darqsz = (ioi->dc_ios - ioj->dc_ios) ?
                          (ioi->dc_sectors - ioj->dc_sectors) /
                          ((double) (ioi->dc_ios - ioj->dc_ios)) :
                          0.0;
        }
        if ((hpart == 4) || !hpart) {
            /* f_await */
...
int main(int argc, char **argv) {
    ...
    while (opt < argc) {
        ...
        if (!strcmp(argv[opt], "-p")) {
            ...
            if (strnlen(argv[opt], sizeof(persistent_name_type)) >=
                sizeof(persistent_name_type) - 1) {
                usage(argv[0]);
            }
